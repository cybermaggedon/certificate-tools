...
# Create the root CA
../go/bin/create-key > root.key || exit 1
../go/bin/create-ca-cert -k root.key -v 180 -E cyberprobe@trustnetworks.com -C US -O "Trust Networks" -N "Trust Networks CA root" > root.pem || exit 1

# Create the Intermediate CA1
../go/bin/create-key > ca1.key || exit 1
../go/bin/create-cert-request -E cyberprobe+ca1@trustnetworks.com -N "Trust Networks CA1" -C US -O "Trust Networks" -k ca1.key > ca1.req || exit 1
../go/bin/create-cert -k root.key -c root.pem -r ca1.req -A -R > ca1.pem || exit 1
rm ca1.req


# Create Client Testuser
../go/bin/create-key > testuser.key || exit 1
../go/bin/create-cert-request -E testuser@trustnetworks.com -N "M. Test User" -C US -O "Trust Networks" -k testuser.key > testuser.req || exit 1
../go/bin/create-cert -k ca1.key -c ca1.pem  -r testuser.req -C > testuser.cert || exit 1
rm testuser.req
...
../go/bin/create-key > baduser.key || exit 1
../go/bin/create-cert-request -E baduser@trustnetworks.com -N "M. Bad User" -C US -O "Trust Networks" -k baduser.key > baduser.req
../go/bin/create-cert -k ca1.key -c ca1.pem  -r baduser.req -C > baduser.cert || exit 1
rm baduser.req
...
../go/bin/create-crl -k root.key -c root.pem -r /dev/null > root.crl || exit 1
openssl crl -verify -CAfile root.pem -in root.crl -noout || exit 1


../go/bin/find-cert -d . -p baduser.cert  > revoke_list
../go/bin/create-crl -k ca1.key -c ca1.pem -r revoke_list >  ca1.crl || exit 1
...
# Test create-rand
N=$(../go/bin/create-rand -b 16 -c 64 | wc -c | sed -e"s/ //g")
if [ "$N" != "1024" ]; then
